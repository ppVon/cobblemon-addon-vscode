import { parseCsvIdentifiers } from '../command-utils';
...
export function validatePokemonBuilderFormData(data: PokemonBuilderFormData): string | undefined {
  ...
  if (data.includeFeatures) {
    const keys = parseCsvIdentifiers(data.featureKeys);
    if (keys.length === 0) {
...
export function isPokemonBuilderFormData(value: unknown): value is PokemonBuilderFormData {
